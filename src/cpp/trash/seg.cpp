...
struct SegmentTreeSP {
  ...
  struct Node {
    ...
    inline void _emplace_childlen() {
      if (!childlen[0][0])
        childlen[0][0].reset(new Node()), analy_counter++;
      if (!childlen[0][1])
        childlen[0][1].reset(new Node()), analy_counter++;
      if (!childlen[1][0])
        childlen[1][0].reset(new Node()), analy_counter++;
      if (!childlen[1][1])
        childlen[1][1].reset(new Node()), analy_counter++;
    }
    ...
    volume_t count_range(int sy, int sx, int ty, int tx, int top, int left, int bottom, int right) {
      ...
      _emplace_childlen();
      return childlen[0][0]->count_range(sy, sx, ty, tx, top, left, centery, centerx) +
