...
struct SegmentTree {
    ...
    inline void set_range(int begin, int end) {
        return _set_range(begin, end, 1, 0, n);
    }
    ...
    inline void reset_range(int begin, int end) {
        return _reset_range(begin, end, 1, 0, n);
    }


    inline int _count_range(int begin, int end, int idx, int left, int right) {
        if (end <= left || right <= begin) return 0; // out of range
        _apply_lazy(idx, right - left);
        if (begin <= left && right <= end) return data[idx];

        int center = (right + left) / 2;
        return
            _count_range(begin, end, idx * 2, left, center)
            + _count_range(begin, end, idx * 2 + 1, center, right);
    }

    inline int count_range(int begin, int end) {
        return _count_range(begin, end, 1, 0, n);
    }
};
