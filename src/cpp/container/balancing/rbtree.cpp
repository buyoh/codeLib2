...
public:
    ...
    struct Node {
        ...
        static inline void insertFixup(unique_ptr<Node>& grand, unique_ptr<Node>& parent, unique_ptr<Node>& node) {
            if (!parent) {
                node->black = 1;
                return;
            }
        ...
        static bool insert(unique_ptr<Node>& grand, unique_ptr<Node>& parent, unique_ptr<Node>& node, const T& newValue) {
            if (!node) {
    ...
    inline bool include(const T& x) const {
        return root ? root->include(x) : false;
    }
