...
struct TreapNode {
    ...
    inline void update() {
        n_node = 1;
        if (childlen[0]) n_node += childlen[0]->n_node;
        if (childlen[1]) n_node += childlen[1]->n_node;
    }
...
public:
    ...
    void _concat(unique_ptr<TreapNode>& treap1, unique_ptr<TreapNode>& treap2) {
        if (!treap2) return;
        if (!treap1) { treap1.swap(treap2); return; }
    ...
    inline void concat(Treap& another) {
        _concat(*this, another);
    }

    // TODO: あまりにも下手
    unique_ptr<TreapNode> _split(unique_ptr<TreapNode>& node, int size) {
        if (size <= 0) return unique_ptr<TreapNode>();
        if (!node->childlen[0]) {
            if (!node->childlen[1]) return move(node);
    ...
    inline Treap split(int size) {
        if (!(*this)) return Treap();
        if ((*this)->n_node <= size) {
            return move(*this);
        }
        unique_ptr<TreapNode> res = _split(*this, size);
        (*this)->update(); return res;
    }


    unique_ptr<TreapNode>& _find(unique_ptr<TreapNode>& node, int index) {
        if (!node)
            return node;
        if (!node->childlen[0])
    ...
    inline unique_ptr<TreapNode>& find(int index) {
        return _find(*this, index);
    }
    // 安全でない
    ...
    inline void erase(int index) {
        if (index < 0 && (*this)->n_node <= index) return;
