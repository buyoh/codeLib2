...
public:
    ...
    void generate(int size, value_t val = init_value_) {
        if (size <= 0 || *this) return ;
        vector<TreapNode::randomizer_t::result_type> rr(size);
        for (auto& x : rr) x = TreapNode::rnd();
        sort(rr.begin(), rr.end());

        function<void(unique_ptr<TreapNode>&, int)> dfs
            = [&size, &rr, &val, &dfs](unique_ptr<TreapNode>& node, int idx1) {
            if (idx1 > size) return;
