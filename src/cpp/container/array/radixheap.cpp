        ...
        for (int i = 1; i < 65; ++i)
            if (!vec[i].empty())
                return *min_element(vec[i].begin(), vec[i].end(),
                    [](const pair<value_type, T>& l, const pair<value_type, T>& r){return l.first < r.first; });
        ...
        if (vec[0].empty()) {
            for (int i = 1; i < 65; ++i) {
                ...
                last_ = min_element(vec[i].begin(), vec[i].end(),
                    [](const pair<value_type, T>& l, const pair<value_type, T>& r) {return l.first < r.first; })->first;
                for (auto& p : vec[i])
